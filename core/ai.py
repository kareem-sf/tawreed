...
from core.provider_registry import (
    get_provider_config as get_provider_config,
)
from core.provider_registry import (
    get_provider_names as get_provider_names,
)
...
def analyze_boq_stream(
    ...
    consumer caught ``StopIteration`` â€” a pattern that is documented
    ...
    Returns a dict with at least these keys:
      ...
      - ``date`` (str, YYYY-MM-DD)
      - ``items`` (dict mapping item-id â†’ work-package name)
      - ``error`` (str or None)
    """
    ...
    if "items" not in parsed_data or not isinstance(parsed_data.get("items"), dict):
        ...
        if parsed_data and "items" not in parsed_data:
            ...
            # like the flat item-id â†’ package map). Otherwise leave
